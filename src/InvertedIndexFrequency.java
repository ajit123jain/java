...
public class InvertedIndexFrequency extends Porter{
    public static Set<String> set =null;   //hashset for stopword
    public static HashMap<String,HashMap<String,Integer>>invert_index  = new HashMap<String,HashMap<String,Integer>>(); //hashmap for inverted index
    public static Porter port=new Porter();
    public static int id=1,p=0;
    public static BufferedReader bufferedReader=null;
    public static FileInputStream inputfilename=null;
    
public static void main(String[] args) throws Exception {
      long startTime = System.currentTimeMillis();         
      File f = new File("MapEnglish.ser");
      boolean bool = false;
      bool = f.exists();                   //if binary file exists or not
      try{
         if(bool==false){                                 //binary file not extists
                File folder = new File("/home/ajit/NetBeansProjects/java/english");  //folder path
...
System.out.println("Time after Write"+(System.currentTimeMillis()-startTime));
        FileOutputStream fos = new FileOutputStream("MapEnglish.ser");           
...
FileInputStream fis = new FileInputStream("MapEnglish.ser");
           ...
           String s;
            ...
            StringTokenizer st = new StringTokenizer(s," ");           // change string as string tokenizer 
                ...
                if (set.contains(str)) {                                // check string is stopword or not if yes then continue
                       ...
                       in.put(file.getName(),1);
                       invert_index.put(ss,in);              //if not put into hashmap 
                        } else
                        {
                            //if(invert_index.containsKey(ss)){
                                if(invert_index.get(ss).containsKey(file.getName())){
                                int h = (int)invert_index.get(ss).get(file.getName());
                                
                               System.out.println("Now h is ="+h++);
                               // h++;
                                invert_index.get(ss).replace(file.getName(),h++);
                                }
                                else{
                                HashMap<String,Integer> inin=invert_index.get(ss);            //get hasset of string 
                                inin.put(file.getName(),1);    
                                }
